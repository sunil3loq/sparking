...
#  
# ####Server log analysis is an ideal use case for Spark.  It's a very large, common data source and contains a rich set of
# information.  Spark allows you to store your logs in files on disk cheaply, while still providing a quick and simple way to
# perform data analysis on them.  This homework will show you how to use Apache Spark on real-world text-based production logs
#  and fully harness the power of that data.  Log data comes from many sources, such as web, file, and compute servers, application
#  logs, user-generated content,  and can be used for monitoring servers, improving business and customer intelligence
# , building recommendation systems, fraud detection, and much more.

# ### How to complete this assignment
#  
# ####This assignment is broken up into sections with bite-sized examples for demonstrating Spark functionality for log processing.
#  For each problem, you should start by thinking about the algorithm that you will use to *efficiently* process the log in a
# parallel, distributed manner. This means using the various
# [RDD](http://spark.apache.org/docs/latest/api/python/pyspark.html#pyspark.RDD) operations along
# with [`lambda` functions](https://docs.python.org/2/tutorial/controlflow.html#lambda-expressions) that are applied at each worker.
#  
...
#  
# * `-`
# ####The "hyphen" in the output indicates that the requested piece of information (user identity from remote machine)
# is not available.
# * `-`
...
# ####This is the first line of the request string from the client. It consists of a three components:
# the request method (e.g., `GET`, `POST`, etc.), the endpoint (a [Uniform Resource Identifier]
# (http://en.wikipedia.org/wiki/Uniform_resource_identifier)), and the client protocol version.
#  
# * `200`
# ####This is the status code that the server sends back to the client. This information is very valuable,
# because it reveals whether the request resulted in a successful response (codes beginning in 2),
# a redirection (codes beginning in 3), an error caused by the client (codes beginning in 4),
# or an error in the server (codes beginning in 5). The full list of possible status codes can be found in the HTTP specification
#  ([RFC 2616](https://www.ietf.org/rfc/rfc2616.txt) section 10).
#  
# * `1839`
# ####The last entry indicates the size of the object returned to the client, not including the response headers.
#  If no content was returned to the client, this value will be "-" (or sometimes 0).
#  
# ####Note that log files contain information supplied directly by the client, without escaping.
#  Therefore, it is possible for malicious clients to insert control-characters in the log files, *so care must be taken
# in dealing with raw logs.*
#  
# ### NASA-HTTP Web Server Log
# ####For this assignment, we will use a data set from NASA Kennedy Space Center WWW server in Florida.
#  The full data set is freely available (http://ita.ee.lbl.gov/html/contrib/NASA-HTTP.html) and contains
#  two month's of all HTTP requests. We are using a subset that only contains several days worth of requests.

# ### **(1a) Parsing Each Log Line**
# ####Using the CLF as defined above, we create a regular expression pattern to extract the nine fields of the log line using
#  the Python regular expression [`search` function](https://docs.python.org/2/library/re.html#regular-expression-objects).
#  The function returns a pair consisting of a Row object and 1. If the log line fails to match the regular expression,
#  the function returns a pair consisting of the log line string and 0. A '-' value in the content size field is cleaned up
# by substituting it with 0. The function converts the log line's date string into a Python `datetime` object using the given
#  `parse_apache_time` function.
