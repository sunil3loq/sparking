...
#   In this lab, we will write code that calculates the most common words in the
# [Complete Works of William Shakespeare](http://www.gutenberg.org/ebooks/100)
# retrieved from [Project Gutenberg](http://www.gutenberg.org/wiki/Main_Page).
...
# We'll define a Python function that returns the word with an 's' at the end of the word.
# Please replace `<FILL IN>` with your solution.  If you have trouble, the next cell has the solution.
...
# Exercises will include an explanation of what is expected, followed by code cells where one cell will have one or more
# `<FILL IN>` sections.  The cell that needs to be modified will have
#  TODO: Replace <FILL IN> with appropriate code` on its first line.  Once the `<FILL IN>` sections are updated and the
# code is run, the test cell can then be run to verify the correctness of your solution.
